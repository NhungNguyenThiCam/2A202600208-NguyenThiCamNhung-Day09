...
def supervisor_node(state: AgentState) -> AgentState:
    ...
    # Routing logic với priority
    route = "retrieval_worker"
    route_reason = "default retrieval route"
    needs_tool = False
    risk_high = False

    # Policy keywords (highest priority)
    policy_keywords = [
        "hoàn tiền", "refund", "flash sale", "license", "subscription",
        "cấp quyền", "access", "level 2", "level 3", "admin access",
        "phê duyệt", "approval", "contractor", "emergency access"
    ]
    
    # SLA/Ticket keywords
    sla_keywords = ["p1", "p2", "sla", "ticket", "escalation", "escalate", "incident"]
    
    # Risk keywords
    risk_keywords = ["emergency", "khẩn cấp", "2am", "3am", "err-", "không rõ", "unknown"]

    # Check policy route (priority 1)
    ...
    # Check SLA route (priority 2) - override if no policy match
    elif any(kw in task for kw in sla_keywords):
        route = "retrieval_worker"
        route_reason = f"task contains SLA/ticket keywords"
        needs_tool = False
    ...
    if matched_risk:
        risk_high = True
        route_reason += f" | risk_high: {', '.join(matched_risk[:2])}"
    
    # Human review override for unknown error codes
    if "err-" in task and "không rõ" not in task:
        # Check if it's a known error pattern
        import re
        err_codes = re.findall(r'err-\d+-\w+', task)
        if err_codes:
            route = "human_review"
            route_reason = f"unknown error code {err_codes[0]} → human review needed"
            risk_high = True
